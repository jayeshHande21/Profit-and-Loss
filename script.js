...
function calculate(initial,current,quantity){
    if(initial > current){
       ...
       ShowMessage(`Hey!! Your total loss is ${loss}, and the LossPercentage is ${lossPercentage.toFixed(2)}% 😔😔😔`);
    }
    else if(current > initial){
        const profit = (current - initial) * quantity;
        const profitPercentage = profit / initial * 100
        ShowMessage(`Hey Your profit is ${profit} and ProfitPercentage is ${profitPercentage.toFixed(2)}%🥳🥳🥳`);
    }else{
